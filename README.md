...
- `.dmg`：`target/release/bundle/osx/DevTools.dmg`，挂载后包含 `DevTools.app` 和 `Applications` 快捷方式，可将应用拖入完成安装。

说明：`make-dmg.sh` 在缺少 `icons/icon.icns` 时会自动调用 `./scripts/make-icons.sh`。
